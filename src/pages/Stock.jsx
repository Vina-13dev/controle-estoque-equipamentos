...
import { archiveItem, createItem, listItems, updateItem } from '../services/itemsService'
...
export default function Stock() {
  ...
  async function removeItem() {
    if (!modal?.item || !isAdmin) return
    setSaving(true)
    setMessage('')
    try {
      await archiveItem(modal.item.id, { uid: profile.id, name: profile.name })
      setModal(null)
      await load()
    } catch (err) {
      console.error(err)
      setMessage(err?.message || 'Não foi possível apagar o item.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <section>
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td className="actions-cell">
                        <button className="icon-button" onClick={() => openEdit(item)} title="Editar"><Edit3 size={17} /></button>
                        {isAdmin && <button className="icon-button danger" onClick={() => openDelete(item)} title="Apagar produto"><Trash2 size={17} /></button>}
                      </td>
                    </tr>
                  )
                })}
      ...
      {modal && modal.type !== 'delete' && (
        <Modal title={modal.type === 'new' ? 'Novo item' : 'Editar item'} onClose={() => setModal(null)} footer={
          ...
          <form id="item-form" className="form-grid" onSubmit={save}>
            ...
            <label className="field"><span>Categoria</span><input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="Ex.: Alimentos ou Limpeza" /></label>
      ...
      {modal?.type === 'delete' && (
        <Modal title="Apagar produto" onClose={() => setModal(null)} footer={
          <><button className="btn ghost" onClick={() => setModal(null)}>Cancelar</button><button className="btn danger" onClick={removeItem} disabled={saving}>{saving ? 'Apagando...' : 'Apagar produto'}</button></>
        }>
          <p><strong>{modal.item.name}</strong> será removido da lista de estoque e não poderá mais receber movimentações.</p>
          <div className="alert info">O histórico de entradas e saídas desse produto continuará preservado nos relatórios.</div>
        </Modal>
      )}
    </section>
  )
